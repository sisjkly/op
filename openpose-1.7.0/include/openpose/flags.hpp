...
DEFINE_string(camera_resolution,        "-1x-1",        "Set the camera resolution (either `--camera` or `--flir_camera`). `-1x-1` will use the"
                                                        ...
                                                        " `--flir_camera`");
//DEFINE_string(video,                    "../../DFEW/Clip/original/data_part_1-2-3/part_1/3.mp4",             "Use a video file instead of the camera. Use `examples/media/video.avi` for our default"
                                                        //" example video.");

DEFINE_string(video,                    "../../Aff-Wild2/video/batch1/20-24-1920x1080.mp4",             "Use a video file instead of the camera. Use `examples/media/video.avi` for our default"
                                                        " example video.");
...
DEFINE_string(output_resolution,        "224x224",        "The image resolution (display and output). Use \"-1x-1\" to force the program to use the"
...
DEFINE_double(face_render_threshold,    0.01,            "Analogous to `render_threshold`, but applied to the face keypoints.");
...
// Result Saving
//DEFINE_string(write_images,             "../../DFEW/Openpose/part1/3",             "Directory to write rendered frames in `write_images_format` image format.");

DEFINE_string(write_images,             "../../Aff-Wild2/Openpose/batch1-output/20-24-1920x1080",             "Directory to write rendered frames in `write_images_format` image format.");
